...
(function () {
  ...
  /* Číslo do atribútu. Odpoveď je JSON zo siete, takže „číslo" v nej môže byť
     hocičo vrátane reťazca s úvodzovkou, ktorý by z data-usage="…" spravil ďalší
     atribút. Na tieto miesta preto nejde hodnota, ale výsledok Number(). */
  function num(x, ak) {
    var n = typeof x === 'number' ? x : parseFloat(x);
    return isFinite(n) ? n : (ak == null ? 0 : ak);
  }
  ...
  /* Slovenčina má tri tvary: 1 kontrola, 2 až 4 kontroly, 5 a viac kontrol.
     Jedna veta s „{n} kontrol" povie pri poslednej kontrole dňa „1 kontrol",
     čo nie je slovenčina. Tvar sa preto vyberá podľa čísla. */
  function mnozne(n) {
    if (n === 1 && T.zostava1) return T.zostava1;
    if (n >= 2 && n <= 4 && T.zostava234) return T.zostava234;
    return T.zostava;
  }
  /* Slovník z konfigurácie sa indexuje kľúčom z odpovede služby, takže „constructor"
     alebo „toString" by vytiahli členov Object.prototype. Escapované sa to nezobrazí
  ...
  function krivka(body, smer) {
    ...
    var v = body.map(function (b) { return (b && typeof b === 'object') ? b.usage_pct : b; }).filter(function (x) { return typeof x === 'number'; });
  ...
  function chybaVety(kod, sprava) {
    return zo(C.err, kod, null) || sprava || T.nedostupne;
  }
  ...
  function vykresli(r, ciastocny) {
    ...
    if (r.changes && r.changes.length) {
      ...
      r.changes.slice(0, 12).forEach(function (z) {
        h.push('<li><time datetime="' + esc(z.ts) + '">' + esc(den(z.ts)) + '</time><div><b>' +
          esc(z.to || z.slug || '') + '</b><p>' + esc(zo(C.kat, z.category, null) || z.category || '') +
          (z.from ? ' <span class="th-od">' + esc(T.z) + ' ' + esc(z.from) + '</span>' : '') + '</p></div></li>');
      });
      h.push('</ul>');
    }

    /* Obal th-scroll musí byť aj tu, nielen pri tabuľkách zo servera: pod 600 px má
       tabuľka nálezov min-width 560 px, takže bez vodorovného posúvača pretiekla
       celá stránka a na 390 px sa dala ťahať do strán. */
    h.push('<div class="th-grid"><div class="th-scroll"><table class="th-nalezy" id="th-nalezy"><caption>' + esc(T.nalezy) +
    ...
    (r.categories || []).forEach(function (k) {
      (k.findings || []).forEach(function (f) {
        prazdne = false;
        var id = 'd' + (i++);
        h.push('<tr><td class="th-kat">' + esc(zo(C.kat, k.id, null) || k.label || k.id) + '</td><td><span class="meno">' +
          esc(f.name || f.slug) + '</span>' + (f.version ? ' <span class="verzia">' + esc(f.version) + '</span>' : '') +
          '</td><td class="th-istota ' + esc(f.confidence || '') + '">' + esc(zo(C.istoty, f.confidence, null) || f.confidence || '') + '</td><td>');
        ...
        if (f.evidence && f.evidence.length) {
          ...
          f.evidence.forEach(function (d) {
            h.push('<li><dfn>' + esc(zo(C.sig, d.type, null) || d.type || '') + '</dfn><code>' +
    ...
    json(API + '/api/changes?limit=8', function (r) {
      ...
      var h = r.items.slice(0, 8).map(function (z) {
        return '<li><time datetime="' + esc(z.ts) + '">' + esc(den(z.ts)) + '</time><div><b>' + esc(z.domain) +
          '</b><p>' + esc(zo(C.kat, z.category, null) || z.category || '') + ': ' + esc(z.to || '') +
          (z.from ? ' <span class="th-od">' + esc(T.z) + ' ' + esc(z.from) + '</span>' : '') + '</p></div></li>';
      }).join('');
      denik.innerHTML = h;
      denik.hidden = false;
  ...
  if (stat && C.live) {
    /* /api/stats/{tld}, nie /api/stats.json: taká cesta v službe nikdy nebola,
       takže táto tabuľka sa zo živej služby nikdy neobnovila a chyba bola tichá,
       lebo 404 má tvar {"ok":false} a ten sa tu potichu ignoruje. */
    var tldStat = stat.getAttribute('data-tld') || 'sk';
    json(API + '/api/stats/' + encodeURIComponent(tldStat), function (r) {
      ...
      data.categories.forEach(function (k) {
        ...
        tab.tBodies[0].innerHTML = k.items.map(function (p) {
          var pouzitie = p.usage_pct == null ? null : num(p.usage_pct);
          var podiel = p.share_pct == null ? null : num(p.share_pct);
          var zmena = p.change_30d_pp == null ? null : num(p.change_30d_pp);
          var n = num(p.n, 0);
          var smer = Math.sign(zmena || 0), hore = smer > 0;
          return '<tr data-nazov="' + esc(p.name) + '" data-usage="' + (pouzitie == null ? -1 : pouzitie) +
            '" data-share="' + (podiel == null ? -1 : podiel) + '" data-zmena="' + (zmena == null ? 0 : zmena) +
            '" data-n="' + n + '"><td class="nazov">' + esc(p.name) + '</td><td class="n">' +
            (pouzitie == null ? '<span class="th-malo">' + esc(T.malo) + '</span>' : pct(pouzitie) + ' %<span class="th-bar"><i style="width:' +
              (max ? Math.round(pouzitie / max * 100) : 0) + '%"></i></span>') + '</td><td class="n">' +
            (podiel == null ? '' : pct(podiel) + ' %') + '</td><td class="n th-zmena ' + (smer > 0 ? 'hore' : smer < 0 ? 'dole' : 'nula') + '">' +
            (zmena == null ? '' : (hore ? '+' : '') + pct(zmena, 1) + ' pb') + '</td><td class="n">' +
            cislo(n) + '</td><td class="n">' + krivka(p.series, smer) + '</td></tr>';
        }).join('');
      });
