...
(function () {
  ...
  V.tok = [
    ...
    '        jas   += (0.024 / (d * d + 0.0040)) * sila;',
    '        ostry += (0.006 / (d * d + 0.0006)) * sila;',
    '      }',
    '    }',
    '  }',
    // tok je hustejší v strede pásu, ktorý ide zľava dole doprava hore
    '  float pas = 1.0 - smoothstep(0.0, 0.62, abs((uv.y - 0.46) - (uv.x - 0.5) * 0.34));',
    '  jas *= 0.55 + 0.85 * pas;',
    '  ostry *= 0.4 + 1.1 * pas;',
    '  vec3 f = mix(UHLIK, ZERAZ, clamp(jas * 1.5, 0.0, 1.0));',
    ...
    '  gl_FragColor = zloz(f, clamp(jas + ostry * 0.7, 0.0, 1.4) * utlm(uv), 0.40);',
    '}',
  ].join('\n');
  ...
  /* hora: cierny masiv proti tlmenemu svetlu a jeden uzky tok energie.

     Co robi siluetu citatelnou: nie je to farba kamena, ale svetlo ZA nim.
     Obloha ma pri horizonte slabu teplu ziaru, kamen je takmer cierny, a
     hrana medzi nimi je tym padom ostra. Presne tak sa fotografuje hora
     proti svitaniu.

     Oranzova je akcent, nie material. Je len na toku a na hrane, ktora je
     k toku privratena. Ked je oranzove vsetko, oranzove nie je nic.

     Vstupy: mys hybe kamerou o par stupnov, posun ju pri scrollovani
     priblizuje, zrych zrychli tok pri hoveri na hlavne tlacidlo. */
  V.hora = [
    'float hrebenH(vec2 p){',
    '  float h = fbm(p * 0.30) * 0.85;',
    '  h += fbm(p * 0.95) * 0.22;',
    '  vec2 a = (p - vec2(1.9, 6.2)) * 0.30;',
    '  h += 3.10 * exp(-dot(a, a));',
    '  vec2 b2 = (p - vec2(-2.6, 8.4)) * 0.22;',
    '  h += 1.70 * exp(-dot(b2, b2));',
    '  vec2 c2 = (p - vec2(5.6, 9.6)) * 0.20;',
    '  h += 1.20 * exp(-dot(c2, c2));',
    '  return h - 1.05;',
    '}',
    'vec3 drahaToku(float u, float ohyb){',
    '  float z = 2.0 + u * 9.0;',
    '  float x = -2.9 + u * 6.2 + sin(u * 3.1) * 0.55 + ohyb;',
    '  float y = -0.55 + u * 2.35 + sin(u * 2.2 + 0.6) * 0.22;',
    '  return vec3(x, y, z);',
    '}',
    'void main(){',
    '  vec2 uv = gl_FragCoord.xy / rozmer.xy;',
    '  vec2 sr = (gl_FragCoord.xy - 0.5 * rozmer.xy) / rozmer.y;',
    '  float otX = (mys.x - 0.5) * 0.045;',
    '  float otY = (mys.y - 0.5) * 0.028;',
    '  vec3 oko = vec3(0.0 + otX * 1.6, 1.30 - otY * 0.8, -2.2 + posun * 3.2);',
    '  vec3 smer = normalize(vec3(sr.x + otX * 0.22, sr.y - 0.085 + otY * 0.22, 1.0));',
    '  float ohyb = (mys.x - 0.5) * 0.7;',
    '  vec2 ziaraStred = vec2(0.68, 0.46);',
    '  float dz = length((uv - ziaraStred) * vec2(1.05, 1.75));',
    '  float ziara = pow(0.58 / (dz + 0.58), 2.6);',
    '  vec3 obloha = mix(vec3(0.024, 0.024, 0.030), vec3(0.86, 0.32, 0.12), ziara);',
    '  obloha += vec3(0.95, 0.42, 0.16) * pow(ziara, 3.4);',
    '  float t = 0.5;',
    '  float trafil = 0.0;',
    '  vec3 bod = oko;',
    '  for (int i = 0; i < 80; i++) {',
    '    bod = oko + smer * t;',
    '    float d = bod.y - hrebenH(bod.xz);',
    '    if (d < 0.010) { trafil = 1.0; break; }',
    '    if (t > 30.0) break;',
    '    t += max(d * 0.50, 0.045);',
    '  }',
    '  float najd = 999.0;',
    '  float najU = 0.0;',
    '  for (int k = 0; k < 24; k++) {',
    '    float u = float(k) / 23.0;',
    '    vec3 q = drahaToku(u, ohyb);',
    '    float dq = length(bod - q);',
    '    if (dq < najd) { najd = dq; najU = u; }',
    '  }',
    '  vec3 farba = obloha;',
    '  if (trafil > 0.5) {',
    '    vec2 e = vec2(0.05, 0.0);',
    '    float hx = hrebenH(bod.xz - e.xy) - hrebenH(bod.xz + e.xy);',
    '    float hz = hrebenH(bod.xz - e.yx) - hrebenH(bod.xz + e.yx);',
    '    vec3 n = normalize(vec3(hx, 2.0 * e.x, hz));',
    '    farba = vec3(0.016, 0.016, 0.020) + vec3(0.020, 0.020, 0.026) * max(n.y, 0.0);',
    '    vec3 kToku = normalize(drahaToku(najU, ohyb) - bod);',
    '    float lem = pow(max(dot(n, kToku), 0.0), 2.2) * exp(-najd * 1.15);',
    '    farba += OHEN * lem * 0.55;',
    '  }',
    '  float rychlost = 0.30 + zrych * 0.85;',
    '  float beh = fract(najU - cas * rychlost * 0.10);',
    '  float pulz = 0.55 + 0.45 * sin((najU * 12.0) - cas * rychlost * 3.2);',
    '  float jadro = exp(-najd * 26.0);',
    '  float lemT  = exp(-najd * 5.5) * 0.30;',
    '  float tokS = (jadro * 1.15 + lemT) * (0.55 + 0.45 * pulz);',
    '  float castice = 0.0;',
    '  for (int m = 0; m < 7; m++) {',
    '    float f = float(m) / 7.0;',
    '    float up = fract(f + cas * rychlost * 0.055);',
    '    vec3 qp = drahaToku(up, ohyb);',
    '    float dp = length(bod - qp);',
    '    castice += 0.010 / (dp * dp + 0.010);',
    '  }',
    '  castice *= 0.55;',
    '  vec3 svetlo = mix(ZERAZ, OHEN, clamp(jadro * 1.3, 0.0, 1.0));',
    '  svetlo = mix(svetlo, JANTAR, clamp(jadro - 0.45, 0.0, 1.0));',
    '  svetlo = mix(svetlo, BIELA, clamp(jadro - 0.86, 0.0, 1.0));',
    '  farba += svetlo * (tokS + castice);',
    '  float hmla = 1.0 - exp(-t * t * 0.0016);',
    '  farba = mix(farba, vec3(0.012, 0.012, 0.016), hmla * 0.88);',
    '  float miesto = 0.10 + 0.90 * smoothstep(0.16, 0.58, uv.x);',
    '  float kryje = clamp(ziara * 1.9 + step(0.5, trafil) * 1.0 + (tokS + castice) * 2.2, 0.0, 1.0);',
    // Tato scena sa NEskláda cez zloz(). Ta funkcia stropuje krytie na STROP
    // a este tlmi k okrajom, co je spravne pri jemnych pozadiach, ale tu to
    // zabije siluetu. Hora si preto riadi krytie sama: vpravo kresli naplno,
    // vlavo, kde je nadpis, sa plynulo vytrati.
    '  float alfa = clamp(kryje, 0.0, 1.0) * miesto;',
