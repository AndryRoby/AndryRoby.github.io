...
function zakladne(ctx) {
  const d = ctx.koren;
  const P = (kod, uzol, hodnota, sk, cs, de, cesta_) => pridaj(ctx, kod, CH, uzol, hodnota, sk, cs, de, cesta_);
...
function riadky(ctx) {
  const P = (kod, uzol, hodnota, sk, cs, de) => pridaj(ctx, kod, CH, uzol, hodnota, sk, cs, de);
  ...
  for (const s of skupiny) {
    for (const ac of s.pole) {
      ...
      if (!maDovod) {
        ...
        P(s.dovodKod, ac, '', sk, cs, de);
        P(s.coKod, ac, '', sk, cs, de);
      }
    }
  }
}
...
function sucty(ctx) {
  const P = (kod, uzol, hodnota, sk, cs, de, c) => pridaj(ctx, kod, CH, uzol, hodnota, sk, cs, de, c);
  const S = ctx.sucty;
  ...
  const povinne = [
    ['BR-12', 'cbc:LineExtensionAmount', 'BT-106', 'súčet suma riadkov bez DPH', 'součet částek řádku bez DPH', 'Summe der Nettobeträge der Positionen'],
    ['BR-13', 'cbc:TaxExclusiveAmount', 'BT-109', 'základ dane spolu', 'základ daně celkem', 'Gesamtbetrag ohne Umsatzsteuer'],
    ['BR-14', 'cbc:TaxInclusiveAmount', 'BT-112', 'suma s DPH', 'částka s DPH', 'Gesamtbetrag mit Umsatzsteuer'],
    ['BR-15', 'cbc:PayableAmount', 'BT-115', 'suma na úhradu', 'částka k úhradě', 'Fälliger Betrag']
  ];
  for (const [kod, prvok, bt, sk, cs, de] of povinne) {
    if (!prve(S, prvok)) {
      P(kod, S, '',
...
const DEC = [
  ['BR-DEC-01', 'zlavaD', 'cbc:Amount', 'BT-92', 'suma zľavy na úrovni dokladu'],
  ['BR-DEC-02', 'zlavaD', 'cbc:BaseAmount', 'BT-93', 'základ zľavy na úrovni dokladu'],
  ['BR-DEC-05', 'priplD', 'cbc:Amount', 'BT-99', 'suma príplatku na úrovni dokladu'],
  ['BR-DEC-06', 'priplD', 'cbc:BaseAmount', 'BT-100', 'základ príplatku na úrovni dokladu'],
  ['BR-DEC-09', 'sucty', 'cbc:LineExtensionAmount', 'BT-106', 'súčet suma riadkov'],
  ['BR-DEC-10', 'sucty', 'cbc:AllowanceTotalAmount', 'BT-107', 'súčet zliav'],
  ['BR-DEC-11', 'sucty', 'cbc:ChargeTotalAmount', 'BT-108', 'súčet príplatkov'],
  ['BR-DEC-12', 'sucty', 'cbc:TaxExclusiveAmount', 'BT-109', 'základ dane spolu'],
  ['BR-DEC-14', 'sucty', 'cbc:TaxInclusiveAmount', 'BT-112', 'suma s DPH'],
  ['BR-DEC-16', 'sucty', 'cbc:PrepaidAmount', 'BT-113', 'zaplatená záloha'],
  ['BR-DEC-17', 'sucty', 'cbc:PayableRoundingAmount', 'BT-114', 'zaokrúhlenie'],
  ['BR-DEC-18', 'sucty', 'cbc:PayableAmount', 'BT-115', 'suma na úhradu'],
  ['BR-DEC-19', 'podsuhrn', 'cbc:TaxableAmount', 'BT-116', 'základ dane v rozpise'],
  ['BR-DEC-20', 'podsuhrn', 'cbc:TaxAmount', 'BT-117', 'suma dane v rozpise'],
  ['BR-DEC-23', 'riadok', 'cbc:LineExtensionAmount', 'BT-131', 'suma riadku bez DPH'],
  ['BR-DEC-24', 'zlavaR', 'cbc:Amount', 'BT-136', 'suma zľavy na riadku'],
  ['BR-DEC-25', 'zlavaR', 'cbc:BaseAmount', 'BT-137', 'základ zľavy na riadku'],
  ['BR-DEC-27', 'priplR', 'cbc:Amount', 'BT-141', 'suma príplatku na riadku'],
  ['BR-DEC-28', 'priplR', 'cbc:BaseAmount', 'BT-142', 'základ príplatku na riadku']
];
...
function desatinneMiesta(ctx) {
  ...
  for (const [kod, kde, prvok, bt, popis] of DEC) {
...
function kodovnik(ctx, kod, uzol, hodnota, nazovKodovnika, kdeSk, kdeCs, kdeDe) {
  ...
  for (const n of deti(d, 'cbc:InvoiceTypeCode')) {
    ...
    if (v !== '' && !K.UNTDID_1001_FAKTURA.has(v)) {
      kodovnik(ctx, 'BR-CL-01', n, v, 'UNTDID 1001 pre faktúru (napríklad 380)',
        'Kód typu dokladu (BT-3)', 'Kód typu dokladu (BT-3)', 'Der Rechnungstyp-Code (BT-3)');
    }
  }
  ...
  for (const n of sCurrency) {
    ...
    if (!K.ISO_4217.has(String(v).trim())) {
      kodovnik(ctx, 'BR-CL-03', n, v, 'ISO 4217 alfa-3 (napriklad EUR)',
        'Atribut currencyID prvku ' + n.meno, 'Atribut currencyID prvku ' + n.meno, 'Das Attribut currencyID von ' + n.meno);
    }
  }
  // BR-CL-04 a BR-CL-05
  for (const n of deti(d, 'cbc:DocumentCurrencyCode')) {
    ...
    if (v !== '' && !K.ISO_4217.has(v)) {
      kodovnik(ctx, 'BR-CL-04', n, v, 'ISO 4217 alfa-3 (napriklad EUR)',
        'Mena faktury (BT-5)', 'Mena faktury (BT-5)', 'Die Rechnungswährung (BT-5)');
    }
  }
  for (const n of deti(d, 'cbc:TaxCurrencyCode')) {
    ...
    if (v !== '' && !K.ISO_4217.has(v)) {
      kodovnik(ctx, 'BR-CL-05', n, v, 'ISO 4217 alfa-3',
        'Mena účtovania DPH (BT-6)', 'Měna účtování DPH (BT-6)', 'Der Steuerwährungscode (BT-6)');
    }
  }
  // BR-CL-06
    ...
    for (const n of deti(ob, 'cbc:DescriptionCode')) {
      ...
      if (v !== '' && !K.UNTDID_2005.has(v)) {
        kodovnik(ctx, 'BR-CL-06', n, v, 'UNTDID 2005 v rozsahu 3, 35, 432',
          'Kód dátumu vzniku daňovej povinnosti (BT-8)', 'Kód data uskutečnění plnění (BT-8)', 'Der Code des Steuerstichtags (BT-8)');
      }
    }
  }
  // BR-CL-07
    ...
    for (const n of deti(dr, 'cbc:ID')) {
      ...
      if (v !== undefined && !K.UNTDID_1153.has(String(v).trim())) {
        kodovnik(ctx, 'BR-CL-07', n, v, 'UNTDID 1153',
          'Atribút schemeID identifikátora predmetu faktúry (BT-18)', 'Atribut schemeID identifikátoru předmětu faktury (BT-18)', 'Das Attribut schemeID der Objektkennung (BT-18)');
      }
    }
  }
  ...
  for (const n of deti(d, 'cbc:Note')) {
    ...
    if (!K.UNTDID_4451.has(kodP)) {
      kodovnik(ctx, 'BR-CL-08', n, kodP, 'UNTDID 4451',
        'Kód predmetu poznámky (BT-21) na začiatku cbc:Note', 'Kód předmětu poznámky (BT-21) na začátku cbc:Note', 'Der Bemerkungs-Betreffcode (BT-21) am Anfang von cbc:Note');
    }
  }
  // BR-CL-10 a BR-CL-11 a BR-CL-21 a BR-CL-26
  const schemaTest = [
    ['BR-CL-10', 'cac:PartyIdentification', 'cbc:ID', K.ICD, 'ISO 6523 ICD', 'Atribút schemeID identifikátora strany (BT-29 alebo BT-46)', 'Atribut schemeID identifikátoru strany (BT-29 nebo BT-46)', 'Das Attribut schemeID der Parteikennung (BT-29 oder BT-46)'],
    ['BR-CL-11', 'cac:PartyLegalEntity', 'cbc:CompanyID', K.ICD, 'ISO 6523 ICD', 'Atribút schemeID registračného čísla (BT-30 alebo BT-47)', 'Atribut schemeID registračního čísla (BT-30 nebo BT-47)', 'Das Attribut schemeID der Registernummer (BT-30 oder BT-47)'],
    ['BR-CL-21', 'cac:StandardItemIdentification', 'cbc:ID', K.ICD, 'ISO 6523 ICD', 'Atribút schemeID štandardného identifikátora položky (BT-157)', 'Atribut schemeID standardního identifikátoru položky (BT-157)', 'Das Attribut schemeID der Standardartikelnummer (BT-157)'],
    ['BR-CL-26', 'cac:DeliveryLocation', 'cbc:ID', K.ICD, 'ISO 6523 ICD', 'Atribút schemeID identifikátora miesta dodania (BT-71)', 'Atribut schemeID identifikátoru místa dodání (BT-71)', 'Das Attribut schemeID der Lieferortkennung (BT-71)']
  ];
  for (const [kod, rodicK, dietaK, zoznam, nazovKod, sk, cs, de] of schemaTest) {
    for (const rodic of vsetky(d, rodicK)) {
      for (const n of deti(rodic, dietaK)) {
        ...
        if (!zoznam.has(t)) kodovnik(ctx, kod, n, v, nazovKod, sk, cs, de);
      }
    }
  }
    ...
    for (const n of deti(cc, 'cbc:ItemClassificationCode')) {
      ...
      if (v !== undefined && !K.UNTDID_7143.has(String(v).trim())) {
        kodovnik(ctx, 'BR-CL-13', n, v, 'UNTDID 7143',
          'Atribút listID klasifikácie položky (BT-158)', 'Atribut listID klasifikace položky (BT-158)', 'Das Attribut listID der Artikelklassifizierung (BT-158)');
      }
    }
  }
    ...
    for (const n of deti(kr, 'cbc:IdentificationCode')) {
      ...
      if (v !== '' && !K.ISO_3166_1.has(v)) {
        kodovnik(ctx, 'BR-CL-14', n, v, 'ISO 3166-1 alfa-2 (napriklad SK, CZ, DE)',
          'Kód krajiny', 'Kód země', 'Der Ländercode');
      }
    }
  }
    ...
    for (const n of deti(kr, 'cbc:IdentificationCode')) {
      ...
      if (v !== '' && !K.ISO_3166_1.has(v)) {
        kodovnik(ctx, 'BR-CL-15', n, v, 'ISO 3166-1 alfa-2',
          'Kód krajiny pôvodu položky (BT-159)', 'Kód země původu položky (BT-159)', 'Der Ursprungslandcode (BT-159)');
      }
    }
  }
    ...
    for (const n of deti(pm, 'cbc:PaymentMeansCode')) {
      ...
      if (v !== '' && !K.UNTDID_4461.has(v)) {
        kodovnik(ctx, 'BR-CL-16', n, v, 'UNTDID 4461 (napríklad 58 pre SEPA prevod)',
          'Kód spôsobu platby (BT-81)', 'Kód způsobu platby (BT-81)', 'Der Zahlungsmittelcode (BT-81)');
      }
    }
  }
    ...
    for (const n of deti(tk, 'cbc:ID')) {
      ...
      if (v !== '' && !K.UNTDID_5305.has(v)) {
        kodovnik(ctx, 'BR-CL-17', n, v, 'UNTDID 5305 (S, Z, E, AE, K, G, O, L, M, B)',
          'Kategória DPH', 'Kategorie DPH', 'Die Umsatzsteuerkategorie');
      }
    }
  }
    ...
    for (const n of deti(tk, 'cbc:ID')) {
      ...
      if (v !== '' && !K.UNTDID_5305.has(v)) {
        kodovnik(ctx, 'BR-CL-18', n, v, 'UNTDID 5305 (S, Z, E, AE, K, G, O, L, M, B)',
          'Kategória DPH riadku (BT-151)', 'Kategorie DPH řádku (BT-151)', 'Die Umsatzsteuerkategorie der Position (BT-151)');
      }
    }
  }
  // BR-CL-19 a BR-CL-20 dovody zliav a priplatkov
  for (const [pole, kod, zoznam, nazovKod, sk, cs, de] of [
    [ctx.zlavyDok.concat(ctx.zlavyRiadkov), 'BR-CL-19', K.UNTDID_5189, 'UNCL 5189', 'Kód dôvodu zľavy', 'Kód důvodu slevy', 'Der Abschlagsgrundcode'],
    [ctx.priplatkyDok.concat(ctx.priplatkyRiadkov), 'BR-CL-20', K.UNTDID_7161, 'UNCL 7161', 'Kód dôvodu príplatku', 'Kód důvodu příplatku', 'Der Zuschlagsgrundcode']
  ]) {
    for (const ac of pole) {
      for (const n of deti(ac, 'cbc:AllowanceChargeReasonCode')) {
        const v = txt(n);
        if (v !== '' && !zoznam.has(v)) kodovnik(ctx, kod, n, v, nazovKod, sk, cs, de);
      }
    }
  }
  ...
  for (const n of vsetky(d, 'cbc:TaxExemptionReasonCode')) {
    ...
    if (v !== '' && !K.VATEX.has(v)) {
      kodovnik(ctx, 'BR-CL-22', n, txt(n), 'CEF VATEX (napriklad VATEX-EU-AE)',
        'Kód dôvodu oslobodenia od DPH (BT-121)', 'Kód důvodu osvobození od DPH (BT-121)', 'Der Code des Steuerbefreiungsgrundes (BT-121)');
    }
  }
  // BR-CL-23 jednotky
  const sUnit = [];
  ...
  for (const n of sUnit) {
    ...
    if (!K.REC20.has(v)) {
      kodovnik(ctx, 'BR-CL-23', n, v, 'UN/ECE Rec. 20 (napríklad C62 pre kus, HUR pre hodinu)',
        'Atribut unitCode prvku ' + n.meno, 'Atribut unitCode prvku ' + n.meno, 'Das Attribut unitCode von ' + n.meno);
    }
  }
  ...
  for (const n of vsetky(d, 'cbc:EmbeddedDocumentBinaryObject')) {
    ...
    if (v !== undefined && !K.MIME_PRILOHY.has(String(v).trim())) {
      kodovnik(ctx, 'BR-CL-24', n, v, 'MIMEMediaType (application/pdf, image/png, image/jpeg, text/csv, xlsx, ods)',
        'Atribút mimeCode prílohy (BT-125)', 'Atribut mimeCode přílohy (BT-125)', 'Das Attribut mimeCode der Anlage (BT-125)');
    }
  }
  ...
  for (const n of vsetky(d, 'cbc:EndpointID')) {
    ...
    if (v !== undefined && !K.EAS.has(String(v).trim())) {
      kodovnik(ctx, 'BR-CL-25', n, v, 'CEF EAS (napríklad 0245 pre slovenské DIČ, 9930 pre nemecké USt-IdNr.)',
        'Atribút schemeID elektronickej adresy', 'Atribut schemeID elektronické adresy', 'Das Attribut schemeID der elektronischen Adresse');
    }
  }
}
