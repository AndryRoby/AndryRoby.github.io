...
function renderRows(){
  ...
  if(!rows.length){target.innerHTML=`<div class="empty"><h2>${all.length?'No requirements match this view.':'Start with one document.'}</h2><p>${all.length?'Try another status or a different supplier name.':'Choose a supplier and name the document you need. Then create their private upload link.'}</p><button class="btn ${all.length?'':'primary'}" data-action="${all.length?'clear-filters':'add'}">${all.length?'Clear filters':'Add your first requirement'}</button>${!all.length?'<p class="help">Already have a spreadsheet? <button class="text-button" data-action="import">Import your CSV</button>.</p>':''}</div>`;return;}
