...
if (mesiace) {
  ...
  if (blok && hotove.length) {
    ...
    for (const h of hotove) {
      ...
      tr.innerHTML = '<td><a href="/games/hares/' + h.d + '/">' + pekneDatum(h.d) + '</a></td><td>' + UROVNE[urovenDna(h.d)].label + '</td><td><b>' + formatCas(h.sec) + '</b></td><td>' + (pomoc.length ? pomoc.join(', ') : 'clean') + '</td>';
      tb.appendChild(tr);
    }
    blok.hidden = false;
  }
