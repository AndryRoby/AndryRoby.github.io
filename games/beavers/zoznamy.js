/* Beavers: the archive, the practice index and the week strip of the guide.
 * Reads what this browser has solved (localStorage, see game.js) and marks it
 * in the lists; on the archive page it also draws the history table and adds
 * the days since the page was last built. This script sends nothing; the
 * account sync of a signed in player is game.js's (it merges the account
 * into this browser, so the lists here show it too). */
...
const stavTriedy = (st) => (st && st.done ? 'hotove' : st && Array.isArray(st.v) && st.v.some((x) => x) ? 'rozohrane' : '');
...
document.querySelectorAll('a[data-sada]').forEach((a) => {
  ...
  for (let k = 1; k <= pocet; k++) {
    ...
    if (st && st.done) hotove++; else if (st && Array.isArray(st.v) && st.v.some((x) => x)) rozohrane++;
  }
  if (hotove || rozohrane) {
...
if (mesiace) {
  ...
  const posledny = odkazy.reduce((a, x) => (x.dataset.den > a ? x.dataset.den : a), '');
  // Days after the last build: the page is static, the calendar is not.
  for (let d = posunDen(posledny || dnes, 1); posledny && d <= dnes; d = posunDen(d, 1)) {
    const m = d.slice(0, 7);
    ...
    li.innerHTML = '<a href="' + adresaDna('/games/beavers/', d, dnes) + '" data-den="' + d + '"><b>' + DNI[denVTyzdni(d)] + ' ' + Number(d.slice(8)) + '</b><small>' + UROVNE[urovenDna(d)].label + '</small></a>';
  ...
  if (blok && hotove.length) {
    ...
    document.getElementById('suhrn').innerHTML = '<b>' + hotove.length + '</b> ' + (hotove.length === 1 ? 'pond' : 'ponds') + ' solved, <b>' + ciste + '</b> of them clean, with no hint and no check'
      + (best ? ', best time <b>' + formatCas(best.sec) + '</b> (' + kratkyDatum(best.d) + ')' : '') + ', average <b>' + formatCas(priemer) + '</b>'
      + (seria ? ', current streak <b>' + seria + (seria === 1 ? ' day' : ' days') + '</b>' : '') + '. All of this is kept in this browser, and in your arling.sk account when you are signed in.';
    ...
    for (const h of hotove) {
      ...
      tr.innerHTML = '<td><a href="' + adresaDna('/games/beavers/', h.d, dnes) + '">' + pekneDatum(h.d) + '</a></td><td>' + UROVNE[urovenDna(h.d)].label + '</td><td><b>' + formatCas(h.sec) + '</b></td><td>' + (pomoc.length ? pomoc.join(', ') : 'clean') + '</td>';
      tb.appendChild(tr);
    }
    blok.hidden = false;
  }
...
/* Guide: this week's strip, the same chips as under the pond. */
...
if (pasikTyzden) {
  ...
  for (let k = 0; k < 7; k++) {
    ...
    if (!buduci) a.href = d === dnes ? '/games/beavers/' : adresaDna('/games/beavers/', d, dnes);
