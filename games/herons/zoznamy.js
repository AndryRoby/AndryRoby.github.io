/* Herons: the archive and the practice index. Reads what this browser has
...
const zacate = (st) => (st && Array.isArray(st.v) && st.v.some((y) => y));
...
if (mesiace) {
  ...
  for (let d = posunDen(posledny, 1); posledny && d <= dnes; d = posunDen(d, 1)) {
    ...
    li.innerHTML = '<a href="/games/herons/' + d + '/" data-den="' + d + '"><b>' + DNI[denVTyzdni(d)] + ' ' + Number(d.slice(8)) + '</b><small>' + UROVNE[urovenDna(d)].label + '</small></a>';
  ...
  if (blok && hotove.length) {
    ...
    document.getElementById('suhrn').innerHTML = '<b>' + hotove.length + '</b> ' + (hotove.length === 1 ? 'marsh' : 'marshes') + ' finished, <b>' + ciste + '</b> of them clean, with no hint and no check'
    ...
    for (const h of hotove) {
      ...
      tr.innerHTML = '<td><a href="' + adresaDna('/games/herons/', h.d, dnes) + '">' + pekneDatum(h.d) + '</a></td><td>' + UROVNE[urovenDna(h.d)].label + '</td><td><b>' + formatCas(h.sec) + '</b></td><td>' + (pomoc.length ? pomoc.join(', ') : 'clean') + '</td>';
      tb.appendChild(tr);
    }
    blok.hidden = false;
  }
...
if (pasikTyzden) {
  ...
  for (let k = 0; k < 7; k++) {
    ...
    if (!buduci) a.href = d === dnes ? '/games/herons/' : '/games/herons/' + d + '/';
