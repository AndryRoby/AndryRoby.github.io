/* Dormice: the archive and the practice index. Reads what this browser has
...
/* A Dormice board is one array of marks per small table, so "started" means a
   mark in any of them, or a clue struck out. */
...
const stavTriedy = (st) => (st && st.done ? 'hotove' : maZnacky(st) ? 'rozohrane' : '');
...
document.querySelectorAll('a[data-sada]').forEach((a) => {
  ...
  for (let k = 1; k <= pocet; k++) {
    ...
    if (st && st.done) hotove++; else if (maZnacky(st)) rozohrane++;
  }
  if (hotove || rozohrane) {
...
if (mesiace) {
  ...
  for (let d = posunDen(posledny, 1); posledny && d <= dnes; d = posunDen(d, 1)) {
    ...
    li.innerHTML = '<a href="/games/dormice/' + d + '/" data-den="' + d + '"><b>' + DNI[denVTyzdni(d)] + ' ' + Number(d.slice(8)) + '</b><small>' + UROVNE[urovenDna(d)].label + '</small></a>';
  ...
  if (blok && hotove.length) {
    ...
    document.getElementById('suhrn').innerHTML = '<b>' + hotove.length + '</b> ' + (hotove.length === 1 ? 'copse' : 'copses') + ' solved, <b>' + bezNapovedy + '</b> without hints'
    ...
    for (const h of hotove) {
      ...
      tr.innerHTML = '<td><a href="' + adresaDna('/games/dormice/', h.d, dnes) + '">' + pekneDatum(h.d) + '</a></td><td>' + UROVNE[urovenDna(h.d)].label + '</td><td><b>' + formatCas(h.sec) + '</b></td><td>' + (h.hints || 'none') + '</td>';
      tb.appendChild(tr);
    }
    blok.hidden = false;
  }
...
/* Guide: this week's strip, the same chips as under the grid. */
...
if (pasikTyzden) {
  ...
  for (let k = 0; k < 7; k++) {
    ...
    if (!buduci) a.href = d === dnes ? '/games/dormice/' : '/games/dormice/' + d + '/';
