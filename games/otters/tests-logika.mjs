 ...
 * The important one: hints never contradict the solution, on rivers of every
...
import { geometria, indexHrany, cluesFromLoop } from './generator.mjs';
import {
...
/* Plays a river with hints only: apply every hint, check it against the
 * solution, stop when the river is finished. Returns how many hints were
...
function hrajNapovedami(p) {
  ...
  while (!jeVyriesene(v, p.clues, p.n)) {
    ...
    assert(h, 'no hint on an unfinished river');
    assert(h.hrany.length > 0, 'hint without sides: ' + h.druh);
    ...
    for (const e of h.hrany) {
      eq(indexHrany(e.typ, e.r, e.c, p.n), e.i, 'hint side index does not match its address');
      assert(v[e.i] === 0, 'hint ' + h.pravidlo + ' touches a side that is already marked');
      if (e.val === 1) assert(sol[e.i] === 1, 'hint ' + h.pravidlo + ' draws a line the river does not use');
      if (e.val === 2) assert(sol[e.i] !== 1, 'hint ' + h.pravidlo + ' crosses a side the river uses');
      v[e.i] = e.val;
    }
...
test('porovnaj counts wrong lines and wrong crosses, never untouched sides', () => {
  const p = zadaniePreDen(PRVY_DEN);
  const v = prazdna(p), sol = ploche(p);
  const jeCiara = sol.indexOf(1), nieCiara = sol.indexOf(0);
  v[jeCiara] = 2; // a cross where the river runs: wrong
...
test('porovnaj is happy with the finished river', () => {
  const p = zadaniePreDen(PRVY_DEN);
...
test('jeVyriesene accepts the generated river and rejects it one side short', () => {
  const p = zadaniePreDen(PRVY_DEN);
  const v = ploche(p);
...
test('jeVyriesene ignores crosses: crossing every side the river skips still finishes it', () => {
  const p = zadaniePreDen(PRVY_DEN);
  const sol = ploche(p);
...
test('jeVyriesene rejects two separate loops even when they satisfy every number', () => {
  // Hand-verified 3 x 3: a little square round the top left patch and
  // another round the bottom right one. Numbers [4,1,0,1,0,1,0,1,4] are
  // satisfied, but two loops are not one river.
  const n = 3, g = geometria(n), v = new Uint8Array(g.E);
  for (const [t, r, c] of [['h', 0, 0], ['h', 1, 0], ['v', 0, 0], ['v', 0, 1],
    ['h', 2, 2], ['h', 3, 2], ['v', 2, 2], ['v', 2, 3]]) v[indexHrany(t, r, c, n)] = 1;
  const clues = [4, 1, 0, 1, 0, 1, 0, 1, 4];
  eq(cluesFromLoop({ h: Array.from(v.slice(0, g.H)), v: Array.from(v.slice(g.V0)) }, n), clues, 'fixture');
  assert(!jeVyriesene(v, clues, n), 'two loops must not count as solved');
});

test('jeVyriesene rejects a branch: three lines at one dot', () => {
  const p = zadaniePreDen(PRVY_DEN);
  const g = geometria(p.n), v = ploche(p);
  // find a dot with two lines and turn on a third side there
  for (let d = 0; d < g.D; d++) {
    let L = 0, volna = -1;
    for (let m = 0; m < 4; m++) { const e = g.dotEdges[4 * d + m]; if (e < 0) continue; if (v[e] === 1) L++; else volna = e; }
    if (L === 2 && volna >= 0) { v[volna] = 1; break; }
  }
  assert(!jeVyriesene(v, p.clues, p.n), 'a branch must not count as solved');
});
...
test('napoveda returns null on a finished river', () => {
  const p = zadaniePreDen(PRVY_DEN);
  eq(napoveda(ploche(p), p.clues, p.solution, p.n), null);
});

const DNI = []; for (let k = 0; k < 14; k++) DNI.push(posunDen(PRVY_DEN, k));
const vysledky = [];
test('hints carry an empty board to the finished river for 14 consecutive days (all sizes), never contradicting the solution and never needing a plain reveal', () => {
  for (const d of DNI) {
    const p = zadaniePreDen(d);
    const r = hrajNapovedami(p);
    vysledky.push({ d, n: p.n, u: p.uroven, ...r });
    assert(r.odhalenia === 0, d + ' needed a plain reveal, but every accepted river should be reachable by the rules alone');
  }
});
test('hints solve the first river of every practice set', () => {
  for (const s of SADY) {
...
test('the daily rivers follow the weekly plan: 5, 5, 6, 6, 7, 8, 9', () => {
  const velkosti = [5, 5, 6, 6, 7, 8, 9];
  for (const d of DNI) {
    const p = zadaniePreDen(d);
...
test('the picked candidate is one of the KANDIDATOV made for that day and its difficulty is measured', () => {
  for (const d of DNI.slice(0, 7)) {
    ...
    assert(p.difficulty.clues > 0 && p.difficulty.clues < p.n * p.n, 'no numbers taken away on ' + d);
  }
});
...
test('zbal and rozbal round-trip and stay reasonably small', () => {
  for (const d of DNI.slice(0, 7)) {
    ...
    assert(jeVyriesene(ploche(r), r.clues, r.n), 'the unpacked river does not finish its own puzzle');
  }
});
test('rozbal refuses a packed puzzle of the wrong shape', () => {
  const z = zbal(zadaniePreDen(PRVY_DEN));
  for (const zly of [{ ...z, c: z.c.slice(1) }, { ...z, h: 5 }, { ...z, v: z.v + '0' }]) {
    let threw = false;
...
console.log('\n  hints per river (reveals = no rule found a step, always 0 for an accepted river):');
