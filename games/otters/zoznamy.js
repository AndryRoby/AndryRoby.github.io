/* Otters: the archive and the practice index. Reads what this browser
 * has finished (localStorage, see game.js) and marks it in the lists; on the
...
if (mesiace) {
  ...
  for (let d = posunDen(posledny, 1); posledny && d <= dnes; d = posunDen(d, 1)) {
    ...
    li.innerHTML = '<a href="/games/otters/' + d + '/" data-den="' + d + '"><b>' + DNI[denVTyzdni(d)] + ' ' + Number(d.slice(8)) + '</b><small>' + UROVNE[urovenDna(d)].label + '</small></a>';
  ...
  if (blok && hotove.length) {
    ...
    document.getElementById('suhrn').innerHTML = '<b>' + hotove.length + '</b> ' + (hotove.length === 1 ? 'river' : 'rivers') + ' finished, <b>' + ciste + '</b> of them clean, with no hint and no check'
    ...
    for (const h of hotove) {
      ...
      tr.innerHTML = '<td><a href="' + adresaDna('/games/otters/', h.d, dnes) + '">' + pekneDatum(h.d) + '</a></td><td>' + UROVNE[urovenDna(h.d)].label + '</td><td><b>' + formatCas(h.sec) + '</b></td><td>' + (pomoc.length ? pomoc.join(', ') : 'clean') + '</td>';
      tb.appendChild(tr);
    }
    blok.hidden = false;
  }
...
/* Guide: this week's strip, the same chips as under the marsh. */
...
if (pasikTyzden) {
  ...
  for (let k = 0; k < 7; k++) {
    ...
    if (!buduci) a.href = d === dnes ? '/games/otters/' : '/games/otters/' + d + '/';
