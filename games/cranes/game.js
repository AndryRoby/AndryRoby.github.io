...
hraUi({ pravidlo: 'Each sandbank gets as many straight walkways as its number. At most two per pair, no crossings, all joined.' });
...
function textZdielania() {
  ...
  const odkaz = rezim === 'cvicenie'
    ...
    : (jeDnes ? 'https://arling.sk/games/cranes/' : trvalaAdresaDna('https://arling.sk/games/cranes/', datum));   // ?d=, so the link still opens after the day's page is gone
...
function ukazPasik() {
  ...
  dni.forEach((d, k) => {
    ...
    if (tag === 'a') a.href = adresaDna('/games/cranes/', d, dnes);
