/* Owls: the archive, the practice index and the guide's week strip. Reads
...
// The stored board holds only the player's own owls, so any owl means started.
const zacate = (st) => !!(st && Array.isArray(st.v) && st.v.some((x) => x));
const stavTriedy = (st) => (st && st.done ? 'hotove' : zacate(st) ? 'rozohrane' : '');
...
if (mesiace) {
  ...
  for (let d = posledny ? posunDen(posledny, 1) : dnes; posledny && d <= dnes; d = posunDen(d, 1)) {
    const m = d.slice(0, 7);
    ...
    li.innerHTML = '<a href="' + adresaDna('/games/owls/', d, dnes) + '" data-den="' + d + '"><b>' + DNI[denVTyzdni(d)] + ' ' + Number(d.slice(8)) + '</b><small>' + UROVNE[urovenDna(d)].label + '</small></a>';
  ...
  if (blok && hotove.length) {
    ...
    document.getElementById('suhrn').innerHTML = '<b>' + hotove.length + '</b> ' + (hotove.length === 1 ? 'roost' : 'roosts') + ' solved, <b>' + ciste + '</b> of them clean, with no hint and no check'
    ...
    for (const h of hotove) {
      ...
      tr.innerHTML = '<td><a href="' + adresaDna('/games/owls/', h.d, dnes) + '">' + pekneDatum(h.d) + '</a></td><td>' + UROVNE[urovenDna(h.d)].label + '</td><td><b>' + formatCas(h.sec) + '</b></td><td>' + (pomoc.length ? pomoc.join(', ') : 'clean') + '</td>';
      tb.appendChild(tr);
    }
    blok.hidden = false;
  }
...
/* Guide: this week's strip, the same chips as under the tree. */
...
if (pasikTyzden) {
  ...
  for (let k = 0; k < 7; k++) {
    ...
    if (!buduci) a.href = d === dnes ? '/games/owls/' : adresaDna('/games/owls/', d, dnes);
