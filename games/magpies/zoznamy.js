/* Magpies: the archive and the practice index. Reads what this browser
...
if (mesiace) {
  ...
  for (let d = posunDen(posledny, 1); posledny && d <= dnes; d = posunDen(d, 1)) {
    ...
    li.innerHTML = '<a href="/games/magpies/' + d + '/" data-den="' + d + '"><b>' + DNI[denVTyzdni(d)] + ' ' + Number(d.slice(8)) + '</b><small>' + UROVNE[urovenDna(d)].label + '</small></a>';
  ...
  if (blok && hotove.length) {
    ...
    document.getElementById('suhrn').innerHTML = '<b>' + hotove.length + '</b> ' + (hotove.length === 1 ? 'picture' : 'pictures') + ' solved, <b>' + bezNapovedy + '</b> without hints'
...
if (pasikTyzden) {
  ...
  for (let k = 0; k < 7; k++) {
    ...
    if (!buduci) a.href = d === dnes ? '/games/magpies/' : '/games/magpies/' + d + '/';
