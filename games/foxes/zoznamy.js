/* Foxes: the archive, the practice index and the guide's week strip. Reads
 * what this browser has solved (localStorage, see game.js) and marks it in
 * the lists; on the archive page it also draws the history table and adds
 * the days since the page was last built. Nothing leaves the browser. */
import { posunDen, denVTyzdni, urovenDna, pekneDatum, kratkyDatum, UROVNE, DNI, MESIACE } from './plan.mjs';
import { todayBratislava } from './generator.mjs';
...
/* A lair is started when a piece, a cross, a note or a struck clue is there. */
function rozohrane(st) {
  if (!st) return false;
  for (const k of ['x', 'm', 'ind']) if (Array.isArray(st[k]) && st[k].some((v) => v)) return true;
  return Array.isArray(st.k) && st.k.some((v) => v >= 0);
}
const stavTriedy = (st) => (st && st.done ? 'hotove' : rozohrane(st) ? 'rozohrane' : '');
...
document.querySelectorAll('a[data-sada]').forEach((a) => {
  ...
  let hotove = 0, zacate = 0;
  for (let k = 1; k <= pocet; k++) {
    const st = nacitaj('foxes:p:' + sada + ':' + k);
    if (st && st.done) hotove++; else if (rozohrane(st)) zacate++;
  }
  if (hotove || zacate) {
    const span = a.querySelector('span');
    if (span) span.textContent += ' · ' + hotove + ' of ' + pocet + ' solved' + (zacate ? ', ' + zacate + ' started' : '');
...
if (mesiace) {
  ...
  for (let d = posunDen(posledny || dnes, 1); posledny && d <= dnes; d = posunDen(d, 1)) {
    ...
    li.innerHTML = '<a href="' + adresaDna('/games/foxes/', d, dnes) + '" data-den="' + d + '"><b>' + DNI[denVTyzdni(d)] + ' ' + Number(d.slice(8)) + '</b><small>' + UROVNE[urovenDna(d)].label + '</small></a>';
  ...
  mesiace.querySelectorAll('a[data-den]').forEach((a) => {
    ...
    if (st && st.done) hotove.push({ d, sec: st.sec || 0, hints: st.hints || 0 });
  ...
  if (blok && hotove.length) {
    ...
    const bezNapovedy = hotove.filter((x) => !x.hints).length;
    ...
    document.getElementById('suhrn').innerHTML = '<b>' + hotove.length + '</b> ' + (hotove.length === 1 ? 'lair' : 'lairs') + ' solved, <b>' + bezNapovedy + '</b> without hints'
    ...
    for (const h of hotove) {
      ...
      tr.innerHTML = '<td><a href="' + adresaDna('/games/foxes/', h.d, dnes) + '">' + pekneDatum(h.d) + '</a></td><td>' + UROVNE[urovenDna(h.d)].label + '</td><td><b>' + formatCas(h.sec) + '</b></td><td>' + (h.hints || 'none') + '</td>';
      tb.appendChild(tr);
    }
    blok.hidden = false;
  }
...
/* Guide: this week's strip, the same chips as under the board. */
...
if (pasikTyzden) {
  ...
  for (let k = 0; k < 7; k++) {
    ...
    if (!buduci) a.href = d === dnes ? '/games/foxes/' : adresaDna('/games/foxes/', d, dnes);
