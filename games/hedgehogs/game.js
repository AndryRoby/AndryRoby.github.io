 ...
 *   <body data-sada="easy-1" data-k="3">  a practice garden
 *   nothing                               today's garden (or ?d=YYYY-MM-DD)
 *
 * The garden itself comes, in this order: from <script type="application/json"
...
hraUi({ pravidlo: 'Two hedgehogs in every row, column and flowerbed. No two may touch, not even at a corner.' });
...
const KLUC = rezim === 'cvicenie' ? 'hedgehogs:p:' + sada + ':' + kSada : 'hedgehogs:' + datum;
// The root address always opens today's garden; once the date is settled,
...
if (rezim === 'den' && !body.dataset.den && jeDnes && !location.search) {
  try { history.replaceState(null, '', '/games/hedgehogs/' + datum + '/'); } catch (e) { /* the address stays generic; the game still works */ }
}
...
async function zTabulky(iso) {
  try {
    const r = await fetch('/games/hedgehogs/dni/' + iso.slice(0, 7) + '.json', { cache: 'no-cache' });
    if (!r.ok) return null;
...
function postavMriezku() {
  ...
  doska.setAttribute('aria-label', 'Garden grid ' + n + ' by ' + n);
  const hotove = doska.querySelectorAll('.b');
  if (hotove.length === n * n) {
    // A built page: the cells are already in the HTML, only their roles are missing.
...
function zapisSeriu() {
  // Only a garden solved on its own day counts: the archive is for practice.
  if (!jeDnes) return;
...
function skontrolujStav() {
  ...
  if (!zle) {
    ...
    track('game_check', { game: 'hedgehogs', wrong: 0 });
    return;
  }
  const popis = [];
  if (p.zleJezky.length) popis.push(p.zleJezky.length + (p.zleJezky.length === 1 ? ' hedgehog' : ' hedgehogs'));
  if (p.zleBodky.length) popis.push(p.zleBodky.length + (p.zleBodky.length === 1 ? ' dot' : ' dots'));
  if (checkStav && !odhalene) {
    odhalene = true;
    for (const i of p.zleJezky) bunky[i].classList.add('chyba');
    for (const i of p.zleBodky) bunky[i].classList.add('chyba');
    stavEl.textContent = 'The marked cells are wrong: ' + popis.join(' and ') + '. Fix them and keep going.';
    checkBtn.textContent = 'Check';
    track('game_check', { game: 'hedgehogs', wrong: zle, revealed: true });
    return;
  }
  checkStav = { zle };
  ...
  track('game_check', { game: 'hedgehogs', wrong: zle, revealed: false });
}
...
function ukazNapovedu() {
  ...
  if (tip) {
    ...
    track('game_hint', { game: 'hedgehogs', kind: t.druh, applied: true });
    if (!skontroluj()) ulozStav();
    ukazStav();
    ukazCas();
    return;
  }
  const h = napoveda(v, zadanie.regions, STARS, zadanie.solution);
  if (!h) return;
  tip = h;
  for (const i of h.jednotka) bunky[i].classList.add('tip-jednotka');
...
/* Auto dots (off by default): the eight neighbours of a hedgehog, and, once
 * a row, column or flowerbed holds its hedgehogs, every other cell in it. */
...
function ukazPasik() {
  ...
  if (rezim === 'cvicenie') {
    ...
    for (let k = 1; k <= s.pocet; k++) {
      const st = nacitaj('hedgehogs:p:' + sada + ':' + k);
      ...
      if (k !== kSada) a.href = '/games/hedgehogs/practice/' + sada + '/' + k + '/';
      a.innerHTML = '<small>No.</small><b>' + k + '</b>';
      a.setAttribute('aria-label', 'Garden ' + k + (st && st.done ? ', solved' : ''));
  ...
  dni.forEach((d, k) => {
    ...
    if (tag === 'a') a.href = adresaDna('/games/hedgehogs/', d, dnes);
...
function pouziNastavenia() {
  ...
  if (bunky.length) oznacKonflikty();
  ukazCas();
}
...
async function spusti() {
  if (jeBuduci) {
    ...
    stavEl.textContent = 'This garden opens on ' + pekneDatum(datum) + ' (Bratislava time). Come back then, or play today’s garden.';
    doska.hidden = true;
  ...
  try {
    ...
    stavEl.textContent = 'The garden could not be prepared. Please reload the page.';
    throw e;
  }
  n = zadanie.n;
  ...
  ukazVsetko();
  pouziNastavenia();
  if (datumEl) datumEl.textContent = rezim === 'cvicenie' ? UROVNE[zadanie.uroven].label + ' practice ' + sada.split('-')[1] + ', garden ' + kSada : pekneDatum(datum);
