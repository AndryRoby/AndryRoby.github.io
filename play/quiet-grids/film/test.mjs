// Test filmu Quiet Grids bez prehliadača: node products/arling-sk/play/quiet-grids/film/test.mjs
//
...
//   5. prst nikdy neprekryje text mimo dosky (titulky, názov, záver),
//   6. partitúra: každá udalosť má konečný čas v [0, dĺžka] a známy nástroj,
//   7. titulky filmu: čas čítania aspoň slová / 3 + 0,5 s; texty bez pomlčiek em a en,
//   8. všetky tri verzie záveru (coming-soon, google-play, obchod) prejdú body 1 až 5 v závere,
//   9. zadania vo filme sú doslovne riadky banky appky a platia ich pravidlá (ťahy, reťazec Hintu,
//      jediné doplnenie poslednej cesty Herons).
//
...
export class FalosnePlatno {
  constructor(w = 300, h = 150) { this.width = w; this.height = h; this.texty = []; this._ctx = null; }
...
// ---------- zadania = riadky banky appky ----------

const { ZAZNAMY } = await import('./hlavolamy.js');
for (const [typ, z] of Object.entries(ZAZNAMY)) {
  const cesta = new URL(`../../../../hlavolamy-android/app/src/main/assets/bank/${z.subor}`, import.meta.url);
  let riadky;
  try { riadky = readFileSync(cesta, 'utf8').split('\n'); } catch { chyby.push(`${typ}: banka ${z.subor} sa nedá prečítať`); continue; }
  if ((riadky[z.riadok - 1] || '').replace(/\r$/, '') !== z.text) chyby.push(`${typ}: záznam nie je riadok ${z.riadok} súboru ${z.subor}`);
}
...
function over(W, H, casy, meno) {
  ...
  for (const t of casy) {
    ...
    const dosk = film.kontrola.telefon(t);
    // záver 14,4 až koniec: text mimo dosky sa nesmie dotknúť obdĺžnika dosky a nesmie byť napoly zamaskovaný
    if (t >= 14.4 && dosk.a > 0) for (const r of hlavne.texty) {
      const cx = (r.x0 + r.x1) / 2, cy = (r.y0 + r.y1) / 2;
      if (cx > dosk.x && cx < dosk.x + dosk.w && cy > dosk.y && cy < dosk.y + dosk.h) continue;
      if (r.x1 > dosk.x && r.x0 < dosk.x + dosk.w && r.y1 > dosk.y && r.y0 < dosk.y + dosk.h) { chyby.push(`${meno} ${W}x${H} t=${t.toFixed(3)}: text záveru „${r.t}“ sa prekrýva s doskou`); if (++zle > 12) break; }
      if (r.orezany) { chyby.push(`${meno} ${W}x${H} t=${t.toFixed(3)}: text záveru „${r.t}“ je napoly zamaskovaný`); if (++zle > 12) break; }
    }
    for (const r of hlavne.texty) {
      ...
      for (const p of hlavne.palce) {
        ...
        if (d < p.r) { chyby.push(`${meno} ${W}x${H} t=${t.toFixed(3)}: prst prekrýva text „${r.t}“`); if (++zle > 12) break; }
      }
    }
    for (const r of hlavne.texty) {
      n++;
...
console.log(`\nOK: ${FORMATY.length} formáty x ${vsetky.length} snímok (+ ${ZAVERY.length - 1} ďalšie závery), ${spolu} textov, najmenšie písmo ${najmensie.toFixed(1)} px, ${film.zvuk.length} zvukov, zadania = riadky banky`);
